...
param_expansion=0 //size of cell expansion in pixels. Default is 10.
def min_nuc_area=0 //remove any nuclei with an area less than or equal to this value
def max_nuc_area=100
nuc_area_measurement='Area µm^2'
def min_nuc_intensity=0.2 //remove any detections with an intensity less than or equal to this value
nuc_intensity_measurement='Hematoxylin: Mean'
...
//Brightfield models
    def pathModel = 'D:/QuPath_Extensions/StarDist_Models/he_heavy_augment.pb'
// IF models
    //def pathModel = 'D:/QuPath_Extensions/StarDist_Models/dsb2018_paper.pb'
//NOTE: .PB MODEL MUST BE USED FOR OPENCV-CUDAd
    //def pathModel = 'D:/QuPath_Extensions/StarDist_Models/dsb2018_heavy_augment.pb'
...
def toDelete2 = getDetectionObjects().findAll {measurement(it, nuc_area_measurement) >= max_nuc_area}
removeObjects(toDelete2, true)
def toDelete3 = getDetectionObjects().findAll {measurement(it, nuc_intensity_measurement) <= min_nuc_intensity}
